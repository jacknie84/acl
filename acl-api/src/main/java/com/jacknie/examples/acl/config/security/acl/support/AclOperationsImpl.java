...
import com.jacknie.examples.acl.config.security.acl.AclOperations;
import com.jacknie.examples.acl.config.security.acl.AclSource;
import com.jacknie.examples.acl.config.security.acl.LookupOperations;
...
import com.jacknie.examples.acl.jpa.acl.oid.ObjectIdentitySource;
...
import com.jacknie.examples.acl.jpa.acl.type.AclClass;
...
@Slf4j
@Builder
public class AclOperationsImpl implements AclOperations, LookupOperations {
     ...
     * @return the primary key or null if not found
     */
    private Long createOrRetrieveClassPrimaryKey(String type, Class<? extends Serializable> idType) {
    ...
    private ObjectIdentity toObjectIdentity(ObjectIdentitySource source) {
        ...
        return oidFactory.createObjectIdentity(identifier, source.getClassName());
    }
    ...
    private <T extends Serializable> T convert(String objectIdIdentity, Class<T> targetType) {
        if (idConversionService.canConvert(String.class, targetType)) {
            return idConversionService.convert(objectIdIdentity, targetType);
        } else {
            return null;
        }
    }
